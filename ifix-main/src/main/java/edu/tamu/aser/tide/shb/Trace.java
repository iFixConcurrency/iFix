...
//	}
	...
	public String toString() {
		int start = 1000000;
		int end = -1;
		for(INode node : this.trace){
			if(node instanceof MethodNode){
				start = Math.min(start, ((MethodNode)node).getLine());
				end = Math.max(end, ((MethodNode)node).getLine());
			}
			else if (node instanceof MemNode){
				start = Math.min(start, ((MemNode)node).getLine());
				end = Math.max(end, ((MemNode)node).getLine());
			}
			else if (node instanceof SyncNode){
				start = Math.min(start, ((SyncNode)node).getLine());
				end = Math.max(end, ((SyncNode)node).getLine());
			}
//			start = Math.min(start, node.)
		}
