...
//	}
		...
		try{
			if(initial){
				...
				for(TIDERace race: bugEngine.races){
					if(race.node1.getPrefix().contains("PersonState") || race.node2.getPrefix().contains("PersonState")){
						System.out.println(race.node1);
						System.out.println(race.node2);
						System.out.println("======");
					}	
	...
	/**
	 * delete race and DeadLock marker
	 * delete previous fix marker
	 * @param project
	 * @throws CoreException 
	 */
	private void deleteMarker(IJavaProject project) throws CoreException {
		IMarker[] markers0 = project.getResource().findMarkers(BugMarker.TYPE_SCARIEST, true, 3);
		IMarker[] markers1 = project.getResource().findMarkers(BugMarker.TYPE_SCARY, true, 3);
		IMarker[] markers2 = project.getResource().findMarkers(BugMarker.TYPE_TROUBLING, true, 3);
		for (IMarker marker : markers0) {
			marker.delete();
		}
		for (IMarker marker : markers1) {
...
//						public void run() {
//							//do update
//							pFixView.ignoreBugs(bugEngine.removedraces);
//						}
//					});
//					break;
//				}
...
//						public void run() {
//							//do update
//							pFixView.considerBugs(bugEngine.addedraces);
//						}
//					});
//					break;
//				}
...
//		trace2.add(l22.toString());
	...
	private LinkedList<String> obtainTraceOfINode(int tid, INode rw1, ITIDEBug bug, int idx) {
		...
		// avoid dead loop
		HashSet<INode> nodeMap = new HashSet<>();
		while(parent != null){
