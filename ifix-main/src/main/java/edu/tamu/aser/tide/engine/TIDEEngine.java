...
import edu.tamu.aser.tide.akkabug.*;
import edu.tamu.aser.tide.nodes.*;
...
import concurrency.ifix.util.BenchmarkUtil;
...
public class TIDEEngine{
	...
	private static Logger logger = LoggerFactory.getLogger(TIDEEngine.class);

	public static boolean IS_PLUGIN = false;

	public static TIDEEngine getEngine(){
		if(IS_PLUGIN && DetectionHub.engine != null){
			return DetectionHub.engine;
		}
		if (!IS_PLUGIN && ReproduceBenchmarks.engine != null) {
			return ReproduceBenchmarks.engine;
		}
		if (!IS_PLUGIN && BenchmarkUtil.engine != null) {
			return BenchmarkUtil.engine;
		}
		logger.info("engine is null : IS_PLUGIN : " + IS_PLUGIN);
		return null;
	}

	public static void setIsPlugin(boolean isPlugin){
		IS_PLUGIN = isPlugin;
	}
	...
	//to check isolates: only for testingd
	...
	public HashSet<String> sharedFields = new HashSet<>();
	public long iStart;
...
//						System.out.println(node.toString());
	...
	public void detectBothBugs(PrintStream ps) {
		...
		// mainEntryNodes 里面是入口方法节点，就是main方法
		for(CGNode main: mainEntryNodes){
			...
			try{//get source code line number and ifile of this inst
				if(main.getIR().getMethod() instanceof IBytecodeMethod){
					...
					if(position instanceof AbstractSourcePosition){
						path = ((AbstractSourcePosition) position).getURL().getFile();
					}
			...
			while(!threadNodes.isEmpty()){
				...
				Trace ttrace = traverseNode(n);//path insensitive traversal
//				System.out.println("???");
...
//				System.out.println();
 	...
 	protected boolean isStaticField(INode node, int iindex) {
 		if(iindex < 0) {
 			return false;
 		}
				...
				if(!method.isSynthetic()){
					try{//get source code line number of this inst
						if(n.getIR().getMethod() instanceof IBytecodeMethod){
							...
							position = n.getMethod().getSourcePosition(inst.iIndex());
...
//				System.out.println(sourceLineNum + " ," + file);
//				System.out.println("=========end=========");
//				System.out.println("inst is : ( " + inst + " )");
				if(inst instanceof SSAFieldAccessInstruction){
//					System.out.println("is instance of SSAFieldAccessInstruction");
					processSSAFieldAccessInstruction(n, method, insts, i, inst, sourceLineNum, file, curTrace, path);
				}else if (inst instanceof SSAArrayReferenceInstruction){
//					System.out.println("is instance of SSAArrayReferenceInstruction");
					processSSAArrayReferenceInstruction(n, method, inst, sourceLineNum, file, curTrace, path);
				}else if (inst instanceof SSAAbstractInvokeInstruction){
//					System.out.println("is instance of SSAAbstractInvokeInstruction");
...
//								System.out.println("Join : " + node.toString());
							...
							//other method calls
							processNewMethodInvoke(n, csr, inst, sourceLineNum, file, curTrace, path);
						}
					}
				}else if(inst instanceof SSAMonitorInstruction){
//					System.out.println("is instance of SSAMonitorInstruction");
//					System.out.println("ref is : " + ((SSAMonitorInstruction) inst).getRef());
					processSSAMonitorInstruction(n, method, inst, sourceLineNum, file, curTrace, path);
					hasSyncBetween = true;
				}
			}
		}
	...
	private void processNewMethodInvoke(CGNode n, CallSiteReference csr, SSAInstruction inst, int sourceLineNum, IFile file, Trace curTrace, String filePath) {
			...
			if(include(declaringclass)){
				...
				if(node.getMethod().isStatic()){
					...
					if(node.getMethod().isSynchronized()){
						...
						// for deadlock
						will = new DLockNode(curTID,instSig, sourceLineNum, null, null, n, inst, file, filePath);
					...
					if(lockedObjects.size()>0){//must be larger than 0
						...
						if(node.getMethod().isSynchronized()){
							String typeclassname = n.getMethod().getDeclaringClass().getName().toString();
							String instSig = typeclassname.substring(1)+":"+sourceLineNum;
							will = new DLockNode(curTID,instSig, sourceLineNum, objectPointer, lockedObjects, n, inst, file, filePath);
	...
	private void processSSAMonitorInstruction(CGNode n, IMethod method, SSAInstruction inst, int sourceLineNum,
			IFile file, Trace curTrace, String filePath) {
...
//		System.out.println(lockValueNumber);
//		System.out.println(lockPointer);
//	    System.out.println(lockObjects);
//	    for(InstanceKey item : lockObjects) {
...
//	    }
		...
		for (InstanceKey instanceKey : lockObjects) {
			...
			if(((SSAMonitorInstruction) inst).isMonitorEnter()){
				will = new DLockNode(curTID, instSig, sourceLineNum, lockPointer, lockObjects, n, inst, file, filePath);
				will.addLockSig(lock);
			}else{
				next = new DUnlockNode(curTID, instSig, sourceLineNum, lockPointer, lockObjects, n);
	...
	private void processSSAArrayReferenceInstruction(CGNode n, IMethod method, SSAInstruction inst, int sourceLineNum,
		...
		String field = "";
		logArrayAccess(inst, sourceLineNum, instSig, curTrace, n, key, instances, file, field, filePath);
	}


	private void processSSAFieldAccessInstruction(CGNode n, IMethod method, SSAInstruction[] insts, int i,
			SSAInstruction inst, int sourceLineNum, IFile file, Trace curTrace, String filePath) {
		...
		if(((SSAFieldAccessInstruction)inst).isStatic()){
			...
			if(baseObjects.size() > 0) {
				logFieldAccess(inst, sourceLineNum, instSig, curTrace, n, staticPointer, baseObjects, sig, file, filePath);
			} else {
				logFieldAccess(inst, sourceLineNum, instSig, curTrace, n, null, null, sig, file, filePath);
			}
		}else{
			...
			logFieldAccess(inst, sourceLineNum, instSig, curTrace, n, basePointer, baseObjects, sig, file, filePath);
		}
	}
...
//								System.out.println("Join : " + node.toString());
	...
	private void logArrayAccess(SSAInstruction inst, int sourceLineNum, String instSig, Trace curTrace, CGNode n,
			PointerKey key, OrdinalSet<InstanceKey> instances, IFile file, String field, String filePath) {
		String sig = "array.";
		if(inst instanceof SSAArrayLoadInstruction){//read
			ReadNode readNode = new ReadNode(curTID,instSig,sourceLineNum,key, sig, n, inst, file, filePath);
			...
			WriteNode writeNode = new WriteNode(curTID,instSig,sourceLineNum, key, sig, n, inst, file, filePath);
			for (InstanceKey instanceKey : instances) {
				String sig2 = sig+ instanceKey.hashCode();
	...
	private void logFieldAccess(SSAInstruction inst, int sourceLineNum, String instSig, Trace curTrace, CGNode n,
			PointerKey key, OrdinalSet<InstanceKey> instances, String sig, IFile file, String filePath) {
		...
		if(inst instanceof SSAGetInstruction){//read
			...
			if(key != null){
				...
				readNode = new ReadNode(curTID,instSig,sourceLineNum,key, sig, n, inst, file, filePath);
			...
			if(key != null){
				...
				writeNode = new WriteNode(curTID,instSig,sourceLineNum,key, sig, n, inst, file, filePath);
			...
			if(curStatus && !preStatus){
				...
				for (Integer tid : tids) {
					...
					if(method.isStatic()){
						...
						will = new DLockNode(tid,instSig, sourceLineNum, null, null, cgNode, inst,file, path);
						will.addLockSig(lock);
						for (DLockNode exist : currentNodes) {
							dLLockPairs.add(new DLPair(exist, will));
						}
						...
						unlock = new DUnlockNode(tid, instSig, sourceLineNum, null, null, cgNode);
						unlock.addLockSig(lock);
					}else{
						int objectValueNumber = inst.getUse(0);
						...
						if(lockedObjects.size() > 0){//must be larger than 0
							//start to record new locks
							will = new DLockNode(tid,instSig, sourceLineNum, objectPointer, lockedObjects, cgNode, inst, file, path);
							for (InstanceKey key : lockedObjects) {
								String lock = key.getConcreteType().getName()+"."+key.hashCode();
								will.addLockSig(lock);
							}
...
//		System.out.println("SIZE of Removed Bugs (in removeBugsRelatedToInterests): " + removedraces.size() + " " + removeddeadlocks.size()) ;
	...
	private void removeKidThreadRelations(Trace old_trace) {
		...
		for (Integer pid : ptids) {//parent id
			//start map
			//kid id of parent id
			HashSet<Integer> kids = old_trace.getKidTidFor(pid);
			//no such relation, deleted in previous round
			if(kids == null)
			{
				continue;
			}
