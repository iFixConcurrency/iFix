...
public class WriteNode extends MemNode{
	...
	public WriteNode(int curTID, String instSig, int sourceLineNum, PointerKey key,
			String prefix, CGNode node, SSAInstruction inst, IFile file, String filePath) {
		super(curTID, instSig, sourceLineNum, key, prefix, node, inst, file, filePath);
		className = super.node.getMethod().getDeclaringClass().toString();
	}
