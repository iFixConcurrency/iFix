...
public class Elevator implements Runnable {
	...
	private boolean[] destinationList = new boolean[Building.MAX_FLOORS]; // of
																			// type
																			// int
	...
	public void summonDestination(int floorNumber)
			throws ElevatorMovingException {
				synchronized (this) {
	...
	public void requestOpenDoor() throws ElevatorMovingException {
		synchronized (this) {
			if (Simulator.debug)
				log.write("requestOpenDoor(" + elevatorID + ") with elevator "
						+ ((motionState == STOPPED) ? "stopped" : "moving"));
	...
	public void start() {
		if (Simulator.debug)
			log = new Logger("Elevator" + elevatorID);
		keepRunning = true;
	...
	public void run() {
		if (Simulator.debug)
			log.write("Starting elevator " + elevatorID);
		while (keepRunning) {
			switch (motionState) {
			case STOPPED:
				if (Simulator.debug)
					log.write("Elevator " + elevatorID + " is stopped");
				if (riders.isEmpty() && !isDestination()) {
					motionDirection = NO_DIRECTION;
					if (Simulator.debug)
						log.write("Elevator " + elevatorID
								+ " is empty and has no destination");
					action(INACTIVE_TIME);
				} else if (isArrived()) {
					if (Simulator.debug)
						log.write("Elevator " + elevatorID + " has arrived on "
								+ currentFloorNumber);
					openDoor();
					closingDoor();
					removeDestination();
	...
	public void leaveElevator(Person person) throws DoorClosedException {
		if (doorState == DOOR_OPEN)
			...
			if (Simulator.debug)
				log.write("Elevator " + elevatorID
						+ " door is closed can not leave.");
			throw new DoorClosedException();
		}
	}

	public void enterElevator(Person person) throws ElevatorFullException,
			DoorClosedException {
		if (Simulator.debug)
			log.write("enterElevator(" + elevatorID + ") with door "
		...
		if (doorState == DOOR_OPEN) {
			if (riders.size() < MAX_OCCUPANCY) {
				...
				if (Simulator.debug)
					log.write("Elevator " + elevatorID + " is full");
				throw new ElevatorFullException();
			}
		} else {
			if (Simulator.debug)
				log.write("Elevator " + elevatorID
						+ " door is closed can not enter.");
			throw new DoorClosedException();
		}
	}
	...
	private void moveDown() {
		if (isDestinationBelow()) {
			if (currentFloorNumber != 1) {
				if (Simulator.debug)
					log.write("move down moves down");
				action(FLOOR_TRAVEL_TIME);
				--currentFloorNumber;
			}
		} else if (isDestinationAbove()) {
			if (Simulator.debug)
				log.write("move down is changing direction");
			motionDirection = MOVING_UP; // someone missed floor change
											// direction
		} else {
			if (Simulator.debug)
				log.write("move down is stopping");
	...
	// doorState = DOOR_CLOSED;
	// }
	private void resetDoorRequest() {
		synchronized (this) {
			requestDoorOpen = false;
		}
	}

	private boolean isRequestDoorOpen() {
		synchronized (this) {
	...
	private void travel() {
		synchronized (this) {
			if (isDestination()) {
				if (Simulator.debug)
					log.write("Elevator has a destination");
				motionState = MOVING;
				if (motionDirection == MOVING_UP) {
					...
					if (Simulator.debug)
						log.write("Moving Down");
					moveDown();
				} else if (isDestinationAbove()) {
					if (Simulator.debug)
						log.write("Setting direction up");
					motionDirection = MOVING_UP;
					moveUp();
				} else if (isDestinationBelow()) {
					if (Simulator.debug)
						log.write("Setting direction down");
					motionDirection = MOVING_DOWN;
					moveDown();
				} else {
					if (Simulator.debug)
						log.write("someone wants on this floor " + currentFloorNumber);
					stopElevator();
				}
			} else {
				if (Simulator.debug)
					log.write("There is no destination");
				motionDirection = NO_DIRECTION;
				motionState = STOPPED;
				action(INACTIVE_TIME);
			}
