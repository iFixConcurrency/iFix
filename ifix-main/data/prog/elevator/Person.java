...
public class Person implements Runnable {
	...
	public void start() {
		...
		if (Simulator.debug)
			log = new Logger("Person" + personID);
		keepRunning = true;
	...
	private void setDestination() {
		// destination = 1+MyRandom.nextInt(building.MAX_FLOORS);
		destination = ((int) (1 + ((Math.random() * 10000) % (building.MAX_FLOORS))));
		if (Simulator.debug)
			log.write("Person: " + personID + " Setting destination: "
					+ destination);
	}
	...
	private void setWaitTime() {
		...
		if (Simulator.debug)
			log.write("Person: " + personID + " Maxwait: " + activityTime);
	}

	private void setWorkingTime() {
		activityTime = 1000 * ((int) (1 + ((Math.random() * 100) % 6)));
		if (Simulator.debug)
			log.write("Person: " + personID + " Business: " + activityTime);
	}
	...
	private boolean wantsToLeave() {
		...
		double i = (Math.random() * 10000);
		if (Simulator.debug)
			log.write("Person: " + personID + " Leaving number (6600): " + i);
		if (i < 6600)
			return true;
		else
			return false;
	...
	private void action() {
		if (Simulator.debug)
			log.write("About to sleep for " + activityTime + " " + getState());
		try {
	...
	private void enterElevator() {
		try {
			...
			activity = RIDING;
			activityTime = elevator.FLOOR_TRAVEL_TIME
					* Math.abs(location - destination);
			action();
	...
	private void resetWaitForElevator() {
		if (Simulator.debug)
			log.write("Person " + personID + " missed elevator "
